...
def train_plasma_controller():
    ...
    # Create PPO model with optimized hyperparameters for improved reward system
    print("Initializing PPO agent...")
    model = PPO(
        "MlpPolicy",                # Multi-layer perceptron policy
        train_env,
        learning_rate=1e-3,         # Higher learning rate for better convergence
        n_steps=2048,               # Doubled steps to collect before update (more stable)
        batch_size=128,             # Larger batch for better gradient estimates
        n_epochs=20,                # More epochs to fully utilize collected data
        gamma=0.99,                 # Discount factor (long-term reward focus)
        gae_lambda=0.95,            # GAE for variance reduction
        clip_range=0.3,             # Slightly higher PPO clipping for stability
    ...
    eval_callback = EvalCallback(
        ...
        eval_freq=5000,             # Evaluate every 5000 steps with new improved reward
        deterministic=True,
        render=False,
        n_eval_episodes=5
    )
    ...
    # Train the agent
    print("Starting training with improved reward system...")
    print("This will take a few minutes - watch the reward improve!")
    print("-" * 40)
    
    model.learn(
        total_timesteps=100000,     # Extended training (5x more) for better convergence
